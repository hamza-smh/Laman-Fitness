...
const MainLayout = () => {
  ...
  // Show progress animation on page 19
  useEffect(() => {
    if (page === 18) {
      setShowProgress(true)
  ...
  // Auto-skip logic for specific diet pages
  useEffect(() => {
    if (page === 26 && userData?.diet !== undefined && userData.diet !== 4 && userData.diet !== 5) {
      setPageValid(26, true)
      userData.lastVisitedPage === 25 ? next() : navigate('/page/25')
    }
  }, [userData, page])

  // Page mapping (Removed page 12)
  ...
  const screen = pageComponents[page] || <h1>Not Found</h1>
  ...
  // Redirect if previous pages aren’t validated
  useEffect(() => {
    if (!validationStatus || Object.keys(validationStatus).length === 0) return
    const currentPage = parseInt(index)
    for (let i = 1; i < currentPage; i++) {
    ...
    <PageGuard pageNumber={page}>
      <div className='container'>
        <div
          className='header'
          style={{
            backgroundColor: shouldShowBlueHeader ? '#ADD8E6' : '#FFF',
          }}
        >
          {page <= 19 && 'Getting Started'}
          {page <= 19 && <GettingStarted />}
        </div>
        ...
        {page === 6 ? (
          ...
          <Testimonial />
        ) : page === 18 && showProgress && userData.lastVisitedPage === 18 ? (
          <YourProgress />
        ) : page === 19 ? (
          <Trying />
        ) : page === 20 ? (
          ...
          <div className='content-wrapper'>
            ...
            <div className='navBtnHolder'>
              ...
              {page < 19 ? (
                ...
                <button className='result-button' onClick={() => navigate('/results')}>
                  See My Results
                </button>
              )}
            </div>
